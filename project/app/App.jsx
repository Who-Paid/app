// Shared Who Paid? app: state + navigation. Rendered into a Frame (device bezel
// or fullscreen). padTop/padBottom feed safe-area CSS vars the screens consume.
const ds = window.WhoPaidDesignSystem_ce3383;
...
function WPApp({ Frame, padTop = '54px', padBottom = '18px' }) {
  const { useState } = React;
  const { todayISO } = window;
  const [tables, setTables] = useState(() => JSON.parse(JSON.stringify(window.WP_DATA.tables)));
  ...
  if (view === 'table' && openId && T(openId)) {
    body = (
      <window.TableScreen
        table={T(openId)} ds={ds} meName={window.WP_DATA.meName}
    ...
    body = (
      <div style={{ height: '100%', overflow: 'auto' }}>
        <window.StartScreen ds={ds} tables={tables} meName={window.WP_DATA.meName}
  ...
  return (
    <Frame>
      <div style={{ position: 'relative', height: '100%', '--wp-pad-top': padTop, '--wp-pad-bottom': padBottom }}>
        ...
        <WPToast msg={toast} />
        {editPerson && (
          <window.EditSheet table={T(edit.tableId)} person={editPerson} ds={ds}
