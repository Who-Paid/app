import { Icon, Confetti, paidLabel } from './util.jsx'

export default function TableScreen({ table, meName, onBack, onPaid, onEditPerson, onAddPerson, onInvite }) {
  const { Avatar, Badge, IconButton } = window.WhoPaidDesignSystem_ce3383

  const order = table.people
  ...
  // Coin rests in open space, not on top of avatar+name
  const landY = (i, H) => {
    const bandH = H / n
    const c = (i + 0.5) * bandH
    const dir = c < H / 2 ? 1 : -1
  ...
  // Bouncing ball physics: ricochets off top/bottom edges, slowing + squashing on impact
